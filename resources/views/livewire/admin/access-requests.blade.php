<div class="pb-20 lg:pb-0">
    ...
    <!-- Filter -->
    <div class="card-premium mb-4 md:mb-6 p-3 md:p-6">
        <div class="grid grid-cols-2 md:flex md:items-center gap-2">
            <button
                wire:click="$set('statusFilter', 'pending')"
                class="px-3 md:px-4 py-2 rounded-lg transition text-xs md:text-sm {{ $statusFilter === 'pending' ? 'bg-pink-vibrant text-cream' : 'text-cream/70 hover:bg-purple-deep' }}">
                Pendientes
            </button>
            <button
                wire:click="$set('statusFilter', 'approved')"
                class="px-3 md:px-4 py-2 rounded-lg transition text-xs md:text-sm {{ $statusFilter === 'approved' ? 'bg-pink-vibrant text-cream' : 'text-cream/70 hover:bg-purple-deep' }}">
                Aprobadas
            </button>
            <button
                wire:click="$set('statusFilter', 'rejected')"
                class="px-3 md:px-4 py-2 rounded-lg transition text-xs md:text-sm {{ $statusFilter === 'rejected' ? 'bg-pink-vibrant text-cream' : 'text-cream/70 hover:bg-purple-deep' }}">
                Rechazadas
            </button>
            <button
                wire:click="$set('statusFilter', 'all')"
                class="px-3 md:px-4 py-2 rounded-lg transition text-xs md:text-sm {{ $statusFilter === 'all' ? 'bg-pink-vibrant text-cream' : 'text-cream/70 hover:bg-purple-deep' }}">
                Todas
            </button>
        </div>
    </div>
    ...
    <div class="space-y-3 md:space-y-4">
        @forelse($requests as $request)
            <div class="card-premium p-4 md:p-6" wire:key="request-{{ $request->id }}">
                <!-- Mobile Layout -->
                <div class="md:hidden">
                    <div class="flex items-start gap-3 mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="font-display text-base text-cream mb-1">{{ $request->user->name }}</h3>
                            <p class="text-cream/70 text-xs truncate">{{ $request->user->email }}</p>
                        </div>
                    </div>

                    <div class="flex flex-col gap-2 text-xs text-cream/60 mb-3">
                    ...
                    @if($request->status === 'pending')
                        <div class="flex gap-2">
                            <button
                                ...
                                class="flex-1 bg-purple-primary hover:bg-purple-dark text-white font-bold py-2 px-3 rounded-lg text-xs transition-colors">
                                Aprobar
                            </button>
                            <button
                                wire:click="rejectRequest({{ $request->id }})"
                                wire:confirm="¿Rechazar solicitud de {{ $request->user->name }}?"
                                class="flex-1 bg-red-500/20 text-red-400 font-bold py-2 px-3 rounded-lg text-xs hover:bg-red-500/30 transition">
                                Rechazar
                            </button>
                        </div>
                    @endif
