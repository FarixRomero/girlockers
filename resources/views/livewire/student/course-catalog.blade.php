        ...
        @if($selectedLevel !== 'all' || $selectedInstructor)
            <div class="flex flex-wrap gap-2 items-center mt-3">
                ...
                @if($selectedInstructor)
                    @php
                        $selectedInstructorName = $instructors->firstWhere('id', $selectedInstructor)?->name ?? 'Instructor';
                    @endphp
                    <button
                        wire:click="$set('selectedInstructor', null)"
                        class="inline-flex items-center px-3 py-1 text-xs md:text-sm font-medium rounded-full bg-purple-600 text-white"
                    >
        ...
        >
            <div class="flex justify-center py-4">
                <div wire:loading wire:target="loadMore" class="flex items-center space-x-2 text-gray-600">
                    ...
                    <span class="text-sm font-medium">Cargando más cursos...</span>
                </div>
            </div>
        </div>
    @endif
